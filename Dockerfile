...
COPY . .
RUN yarn install --build-from-source && yarn build
...
RUN <<EOF
apt-get update
apt-get install -y --no-install-recommends busybox-static=1:1.30.1-7ubuntu3 ca-certificates=20230311ubuntu0.22.04.1 curl=7.81.0-1ubuntu1.15 sqlite3
...
COPY --from=build-stage /opt/cartesi/dapp/node_modules/sqlite3/build/Release/node_sqlite3.node /opt/cartesi/dapp/build/
